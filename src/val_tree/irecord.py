...
def MATCHES(r, flags=0):
    return lambda x: re.fullmatch(r, str(x).strip(), flags)
...
ROW_VALIDATOR = cl.OrderedDict({
    ...
    'S/P'                               : util.make_validator(
        ...
        util.any_fn(OPTIONAL(), MATCHES(r'S|P', re.IGNORECASE))),
    ...
    'obvod kmene [cm]'                  : util.make_validator(
        ...
        util.any_fn(OPTIONAL(), MATCHES(r'\d+([,;]\d+)*[,;]?'))),

    'výška stromu [m]'                  : util.make_validator(
        f'"výška stromu [m]" must be positive',
        util.any_fn(OPTIONAL(), MATCHES(r'(\d*\.\d+)|(\d+\.?)'))),
    ...
    'vitalita'                          : util.make_validator(
        ...
        util.any_fn(OPTIONAL(), MATCHES(r'[1-5]'))),
    ...
    'poškození borky (A)'               : util.make_validator(
        ...
        util.any_fn(OPTIONAL(), MATCHES(r'A', re.IGNORECASE))),
...
def parse(col_it):
    r = dict(zip(ROW_VALIDATOR.keys(), col_it))
    return (r, ROW_CHECKER(r))
